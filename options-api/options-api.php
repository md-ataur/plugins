...
Text Domain: option-api
*/

/* Enqueue Scripts */
add_action( 'admin_enqueue_scripts', function ( $hook ) {
    if ( 'toplevel_page_options-api' == $hook ) {
        wp_enqueue_style( 'pure-grid-css', '//unpkg.com/purecss@1.0.1/build/grids-min.css' );
        wp_enqueue_style( 'options-api-css', plugin_dir_url( __FILE__ ) . "assets/css/style.css", null, time() );
        wp_enqueue_script( 'options-api-js', plugin_dir_url( __FILE__ ) . "assets/js/main.js", array( 'jquery' ), time(), true );
...
add_action( 'wp_ajax_options_display_result', function () {
    ...
    if ( wp_verify_nonce( $_POST['nonce'], 'options_display_result' ) ) {
        ...
        /* Update data. If the key set, the value will be updated. If the key doesn't set, the key will be inserted with value */
...
// You can use Filter hook to change data
...
add_action( 'admin_menu', function () {
    add_menu_page( 'Options Api', 'Options Api', 'manage_options', 'options-api', 'optionsapi_admin_page' );
} );

function optionsapi_admin_page() {
    ?>
