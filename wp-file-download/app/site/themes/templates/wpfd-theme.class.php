...
class WpfdTheme
{
    ...
    public static function outputContentHeader($theme)
    {
        ...
        if ($showBreadcrumb === 1 && !$theme->latest) {
            if ($theme->config['download_category'] && !$theme->categoryFrom) {
                $output .= sprintf(
            ...
            if ($showDownloadAll && !$theme->categoryFrom) {
                $output .= sprintf(
                    '<a data-catid=""
    ...
    public static function showIconHandlebars($config, $params)
    {
        ...
        if ($config['custom_icon']) {
            ...
            $html = '<div class="ext {{ext}}"><span class="txt">{{ext}}</span></div>';
        }
        /**
    ...
    public static function showIcon($file, $config, $params)
    {
        ...
        if ($config['custom_icon'] && isset($file->file_custom_icon) && $file->file_custom_icon !== '') {
            ...
            $html = sprintf(
                '<div class="ext %s"><span class="txt">%s</div>',
    ...
    public static function showTitleHandlebars($config, $params)
    {
        ...
        if ((int) WpfdBase::loadValue($params, self::$prefix . 'showtitle', 1) === 1) {
            $name     = self::$themeName;
            $template = array(
                'html' => '<h3>' . $selectFileInput . '<a href="%url$s" class="wpfd_downloadlink" title="%title$s">%text$s</a></h3>',
                'args' => array(
                    'url'   => '{{linkdownload}}',
                    'title' => '{{post_title}}',
                    'text'  => '{{crop_title}}'
