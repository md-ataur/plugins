...
function wpfd_register_post_type()
{
    ...
    register_post_type(
        ...
        array(
            ...
            'map_meta_cap' => false,
            'capabilities' => array(
                'wpfd_create_category' => esc_html__('Create category', 'wpfd'),
                'wpfd_edit_category' => esc_html__('Edit category', 'wpfd'),
                'wpfd_edit_own_category' => esc_html__('Edit own category', 'wpfd'),
                'wpfd_delete_category' => esc_html__('Delete category', 'wpfd'),
...
function wpfd_init()
{
    ...
    wp_enqueue_script(
        'jquery-ui-1.11.4',
        plugins_url('assets/js/jquery-ui-1.11.4.custom.min.js', __FILE__),
        array(),
        $ver
    );

    if ($page !== 'wpfd-config' && $page !== 'wpfd-notification') {
...
//    wp_enqueue_style('wpfd-bootstrap', plugins_url('assets/css/bootstrap.min.css', __FILE__), array(), $ver);
    ...
    wp_enqueue_script('jquery-tagit', plugins_url('assets/js/jquery.tagit.js', __FILE__), array(), $ver);
    ...
    wp_enqueue_script('wpfd-main', plugins_url('assets/js/wpfd.js', __FILE__), array(), $ver);
...
if (defined('WPFD_ADMIN_UI') && WPFD_ADMIN_UI === true) {
    ...
    include_once $app->getPath() . DIRECTORY_SEPARATOR . 'admin' . DIRECTORY_SEPARATOR . 'ui-functions.php';
}
...
add_action('admin_print_scripts', function () {
    ...
    if ((!empty($_GET['page']) && in_array($_GET['page'], array('wpfd-setup', 'wpfd', 'wpfd-config')))) {
        if (is_user_admin()) {
