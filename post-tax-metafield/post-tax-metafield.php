...
if (!class_exists("PostTaxMeta")) {
	...
	class PostTaxMeta{
		...
		function __construct(){
			...
			add_action( "admin_menu", array( $this,"ptmf_post_meta_add" ));
		...
		function ptmf_load_textdomain(){
			load_plugin_textdomain( "post-tax-metafield", false, plugin_dir_path( __FILE__ )."/languages" );
		}
		...
		private function is_check($nonce_field,$nonce_action,$post_id){
			$nonce = isset($_POST['ptmf_nonce_field']) ? $_POST['ptmf_nonce_field'] : '';
			if ("" == $nonce) {
				return false;
			}
			if (!wp_verify_nonce( $nonce, "ptmf_action" )) {
				return false;
			}
		...
		function ptmf_post_meta_add(){
			add_meta_box( "ptmf_post_metaid", __("Post Meta Field","post-tax-metafield"), array($this,"ptmf_post_meta_display"), array("page"), "normal");
		}

		/* Meta field display */
		function ptmf_post_meta_display($post){
			...
			$label1 = __("Select Post","post-tax-metafield");
