...
Text Domain: transient-api
*/

/* Enqueue Scripts */
add_action( 'admin_enqueue_scripts', function ( $hook ) {
    if ( 'toplevel_page_transient-api' == $hook ) {
        wp_enqueue_style( 'pure-grid-css', '//unpkg.com/purecss@1.0.1/build/grids-min.css' );
        wp_enqueue_style( 'transient-api-css', plugin_dir_url( __FILE__ ) . "assets/css/style.css", null, time() );
        wp_enqueue_script( 'transient-api-js', plugin_dir_url( __FILE__ ) . "assets/js/main.js", array( 'jquery' ), time(), true );
...
add_action( 'admin_menu', function () {
    add_menu_page( 'Transient Api', 'Transient Api', 'manage_options', 'transient-api', 'transientapi_admin_page' );
} );

function transientapi_admin_page() {
    ?>
